...
| тека | що всередині |
|---|---|
| [`powerbi/`](powerbi/) | дві теми оформлення + покрокова інструкція зі збірки дашборда |
...
У теці [`powerbi/`](powerbi/) лежать:

- `obrio-light.json` і `obrio-dark.json` — теми оформлення на основі **справжніх дизайн-токенів
  obrio.co**;
- [`powerbi/README.md`](powerbi/README.md) — покрокова інструкція: які візуали ставити на дві
  сторінки, які поля куди класти й що писати в заголовках.

### Як підключити тему

1. Завантажте `powerbi/obrio-light.json`
2. У Power BI Desktop: **Вигляд → Теми → Огляд тем**
3. Оберіть завантажений файл

Світла версія — для дашборда, який читають і друкують. Темна — для показу на екрані:
фон `#181818` — фірмовий фон OBRIO.

> Брендова аква `#77E5E9` на білому тлі дає контраст лише **1,6 : 1** і нечитабельна,
> тому світла тема використовує глибші кроки тих самих відтінків. Обидві палітри перевірені:
> контраст ≥ 3 : 1, розрізнення сусідніх кольорів для дальтонізму ΔE ≥ 8.

### Файл дашборда

`.pbix` у репозиторій поки не доданий: модель наповнена мірами через підключення до запущеного
Power BI Desktop, і ці зміни зʼявляться у файлі **лише після збереження**.

Щоб додати: збережіть файл у Power BI Desktop (`Ctrl+S`), скопіюйте його в теку `powerbi/`
і виконайте з кореня репозиторію:

```bash
git add powerbi/Obrio_dashboard.pbix
git commit -m "Power BI: модель із мірами фреймворка"
git push
```

Що вже є в моделі: **41 міра** у теках (Гроші, Воронка, Обсяг, Фреймворк, Зони рішення,
Інсайти), обчислювана таблиця `Креативи` (один рядок = один креатив), таблиця-параметр
`Поріг витрат` і колонки `Накопичені витрати / ROMI`, `Перетин $50 / $200`, `Зона на $50 / $200`.
Усі показники звірені з ноутбуком **до цента**.
